from tensorflow.keras.layers import Input, Dense, Lambda, Concatenate, Dropout, LeakyReLU
...
class Manne:
	...
	def load_dataset(self):
		global TRAINING_SIZE
		global VALIDATION_SIZE
		filename = 'frames/'+self.filename+'.npy'	#Static Data used for training net
		...
		train_frames = int(len(self.frames) * TRAINING_SIZE)
		test_frames = train_frames + int(len(self.frames) * VALIDATION_SIZE)
		self.X_train = self.frames[:train_frames,:]
		self.X_val = self.frames[train_frames:test_frames,:]
		self.X_test = self.frames[test_frames:,:]
	...
	def define_net(self):
		...
		if self.net_type == 'vae':
			z_mean = Dense(self.encoder_widths[-1],input_shape=(self.encoder_widths[-1],), name='z_mean')(encoded)
			z_log_var = Dense(self.encoder_widths[-1],input_shape=(self.encoder_widths[-1],), name='z_log_var')(encoded)
			z = Lambda(self.sampling,output_shape=(self.encoder_widths[-1],), name='z')([z_mean, z_log_var])
	...
	def train_net(self):
		...
		self.encoder.save('models/'+self.net_type+'_'+self.filename+'_trained_encoder.h5')
		self.decoder.save('models/'+self.net_type+'_'+self.filename+'_trained_decoder.h5')
	...
	def evaluate_net(self):
		...
		if args.filename == 'one_octave':
			mod = 1
		elif args.filename == 'five_octave' or args.filename == 'violin':
			mod = 10
		elif args.filename == 'guitar':
			mod = 3
		else:
			mod = 1
